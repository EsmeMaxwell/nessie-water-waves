function [ st_Dn ] = fn_ww__setup__diffmtrx__WR_chebdif( N, b_nst )
%fn_ww__setup__diffmtrx__WR_chebdif: Generate diff matrices using WR chebdif
%
%   [ st_Dn ] = fn_ww__setup__diffmtrx__WR_chebdif( N, b_nst )
% 
% Constructs the first and second order differentiation matrix, of size
% (N+1), using chebdif from the Weideman-Reddy DM suite. The negative sum
...
[ v_z0, a_DM ] = fn_ww__ext__diffmtrx__WR_chebdif( N+1, 2 );
a_D = a_DM(:,:,1);
a_D2 = a_DM(:,:,2);
