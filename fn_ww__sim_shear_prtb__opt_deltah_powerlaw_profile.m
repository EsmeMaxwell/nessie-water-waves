function fn_ww__sim_shear_prtb__opt_deltah_powerlaw_profile(  )
%fn_ww__sim_shear_prtb__full_powerlaw_profile: Sim manager to run err-in-shear for powerlawonential profile to find optimal delta h
%
%   fn_ww__sim_shear_prtb__opt_deltah_powerlaw_profile(  )
%
% Manager function to setup and execute the err-in-shear-profile simulation
% for the powerlawonential profile, this time to find optimal delta h.
% 
% See also
...
for lp_Fr2=1:NFr2
    ...
    phy_U0 = sqrt( v_Fr2(lp_Fr2) * st_p.phy_g * phy_h );
    ...
    for lp_hs=1:Nhs
        for lp_dh=1:Ndeltah
            for lp_dU=1:NdeltaU
                ...
                s_filename = sprintf( 'data_shear_prtb_opt_deltah_powerlaw/shear_prtb_powerlaw_opt_deltah__%d_%d_%d_%d', lp_Fr2, lp_hs, lp_dh, lp_dU );
                s_backup_filename = sprintf( 'data_shear_prtb_opt_deltah_powerlaw/shear_prtb_powerlaw_opt_deltah__data__%d_%d_%d_%d', lp_Fr2, lp_hs, lp_dh, lp_dU );
                ...
                % Run the test
                tic
                [ st_err_run ] = fn_ww__sim_shear_prtb__do_runs( st_fn_shear_pwr, v_zs, v_zs_err_width, v_k, v_percentile, Nitr, Nz, st_p, s_filename, s_backup_filename );
                toc;
                ...
                save( 'data_shear_prtb_opt_deltah_powerlaw/shear_prtb_powerlaw_opt_deltah__param_space', 'st_param_save' );
